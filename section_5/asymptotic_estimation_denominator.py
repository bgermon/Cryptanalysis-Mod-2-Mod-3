...
def calculate_exact_denominator(n, a, W, k0, dp_table):
    P_star1 = Decimal(0)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python test_denominator.py n a")
        exit()
    ...
    print(f"Testing Denominator for n = {n}, alpha * n = {a}. Please wait...")
    ...
    for b in tqdm(weights):
        ...
        exact_den = calculate_exact_denominator(n, a, b, k0, dp_table)
        approx_den = asymptotic_denominator(n, alpha, beta, k0)
        
        exact_biases.append(float(exact_den - Decimal('0.5')))
        approx_biases.append(float(approx_den - Decimal('0.5')))
    ...
    ax1.set_ylabel(r'Denominator Bias', fontsize=10)
    ax1.set_xlabel(r'$\beta n$', fontsize=10)
    ax1.set_title(f'Denominator Bias Oscillations $(n={n}, \\alpha n={a}, k_0={k0})$', fontsize=14)

...
def calculate_bias(n, a, W, k0, dp_table):
    P00 = Decimal(0)
    P_star0 = Decimal(0)
...
def asymptotic_bias(n, a, b, k0):
    ...
    num = asymptotic_numerator(n_dec, alpha, beta, k0)
...
print(f"Calculating for n = {n}, alpha * n = {a}. Please wait...")
dp_table = get_all_dp(n)
weights = np.arange(0, n + 1)
from tqdm import tqdm

k0 = 0
biases = np.array([calculate_bias(n, a, b, k0, dp_table) for b in tqdm(weights)])
asymp_biases = np.array([asymptotic_bias(n, a, b, k0) for b in tqdm(weights)])
...
ax1.set_title(f'Bias Oscillations $(n={n}, \\alpha n={a})$', fontsize=14)
...
ax2.plot(weights, magnitude, color='teal', label=f'$\\log_{2}(|\\gamma(n, \\alpha, \\beta, k_0)|), k_0={k0}$', 
